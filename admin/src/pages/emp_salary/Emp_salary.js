...
function Emp_salary() {
  ...
  const generatePDF = () => {
    ...
    doc.text(`Total Salary: ${report.total.toFixed(2)}`, 14, 60);
    ...
    doc.autoTable({
        ...
        body: report.report.map(item => [
            item.date,
            item.total_salary.toFixed(2),
            item.day_salary.toFixed(2),
            item.ot_salary.toFixed(2),
            item.working_time,
        ]),
        styles: {
            fontSize: 12,
    ...
    <div className="salary-report-container">
      ...
      {report && (
        <div className="report-section">
          ...
          <h2>Total Salary: {report.total.toFixed(2)}</h2>
          <h3>Report Details:</h3>
          <ul className="report-list">
            {report.report.map((item, index) => (
              <li key={index} className="report-item">
                <p>Date: {item.date}</p>
                <p>Total Salary: {item.total_salary.toFixed(2)}</p>
                <p>Day Salary: {item.day_salary.toFixed(2)}</p>
                <p>OT Salary: {item.ot_salary.toFixed(2)}</p>
