            ...
            User.findOne({ _id: userId }).then((doc) => {
                ...
                if (doc != null) {
                    ...
                    if (userType === "employee") {
                        ...
                        Attend.find({ user_id: userId })
                            .where("start_time").gte(requestMillis).lt(todayMillis)
                            .where("end_time").ne(0)
                        ...
                        res.send({ "status": "invalid_user_type", "message": "Only employees' salaries can be processed." });
                    }

                } else {
                    res.send({ "status": "user_not_found", "message": "User not found." });
                }

            }).catch((err) => {
                res.send({ "status": "error", "message": "Error retrieving user." });
            });
